...
### 2.1 Insight
Trên service graph, mũi tên A → B nghĩa là A GỌI B. Khi B hỏng, A cũng hỏng theo. Tức là lỗi lan NGƯỢC chiều mũi tên — từ service bị gọi → các service đang gọi nó.

Graph traversal — bắt đầu từ alert sâu nhất, đi ngược chiều mũi tên. Nếu cluster có nhiều service đang alert, ai ở vị trí “sâu nhất” (không gọi ai trong số đang alert) thường là culprit.
...
Idea PageRank: node được “vote” bởi các node link tới nó. Trong RCA, chạy PageRank trên reverse graph — service được nhiều caller phụ thuộc → score cao.
...
Granger causality không phải causation theo nghĩa philosophy — nó là temporal precedence + predictive. Nhưng trong AIOps đủ tốt.
...
- Yêu cầu stationary — series không có trend / seasonality. Phải `np.diff()` trước, hoặc STL decompose.
...
PC algorithm (Peter-Clark) build causal DAG từ data. Library: causal-learn, pgmpy. Đây là research direction, không phải go-to cho production AIOps.
...
## 4. LLM-Augmented RCA
Đến đây bạn có top-3 candidate. LLM giúp:
...
### 4.1 Pipeline tổng
LLM-augmented RCA pipeline gồm 5 stage từ cluster đến structured output.
...
Nâng cao: sentence-transformer embed → cosine similarity → top-K.

### 4.3 Prompt template
Prompt phải có 4 phần: system instruction, context, examples (similar incidents), task.
...
2. Gọi LLM với temperature=0.2 + response_format=json_object / structured output schema
3. Log toàn bộ prompt + raw response vào file để debug + audit
Set timeout=10.0 và max_retries=2.

### 4.5 Hallucination guard
LLM có thể trả root_cause = service KHÔNG có trong cluster. Validate trước khi dùng:
- `root_cause` ∈ `cluster.services`
- `class` ∈ enum đã định nghĩa
- `confidence` ∈ [0, 1]
- `actions` là list non-empty

Nếu invalid → fallback: top-1 từ graph RCA + class="other" + actions=["Investigate manually"].

### 4.6 Cost awareness
gpt-4o-mini: Prompt ~2500 tokens × 1 call = $0.0004. Rẻ nhưng chỉ nên gọi per-cluster.

---

## 5. Putting It Together — Final RCA Function
Pipeline lái logic theo flowchart:
- Tính candidates = `rca_combined(cluster, alerts, graph)` — luôn chạy
- Build graph context (subgraph cluster + service edges) — luôn chạy
- Nếu có LLM enabled → gọi `call_llm()`, validate. Output OK → return. Output sai/timeout → fallback
...
|---|---|---|
| Dynatrace Davis | Topology là source of truth → causal AI dựa trên service map | Auto-detect dependency, top-1 root cause < 5s cho stack đã instrument |
| BigPanda | ML cluster alerts trước, RCA bằng pattern matching | Multi-vendor alert ingestion, dedup ở quy mô lớn |
| Moogsoft | “Situations” — gom alert thành unit có nghĩa | Noise reduction, alert correlation thuần |
| Datadog Watchdog | Bundle anomaly + correlation + APM trong cùng nền tảng | Full-stack observability, native integration |
| Causely | Causal AI học causal graph từ data không cần topology giả định | Microservices không có service map rõ ràng |
| Prometheus AlertManager | Dedup + grouping ở mức routing — không claim RCA | Foundation cho self-built correlator |
| Robusta | Alert smart-routing + auto-remediation playbook | K8s-native, auto-fix common issues |
| Grafana Sift | RCA workflow tích hợp dashboard | Khi dùng Grafana stack đầy đủ |
...
- **Bonus 2 — TF-IDF embedding**: dùng `sklearn.feature_extraction.text.TfidfVectorizer` trên summary + services → cosine similarity → top-K. Compare với keyword retrieval.
- **Bonus 3 — LLM enrichment**: dùng Groq free tier hoặc paid OpenAI/Anthropic — thay step 6 bằng LLM call với prompt structure §4.3. Compare class label vs kNN top-1.
...
- “You Don’t Have a Search Problem, You Have a Ranking Problem” — Charity Majors, Honeycomb blog
