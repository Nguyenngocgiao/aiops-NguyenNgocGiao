...
def main():
    ...
    if args.auto_approve:
        ...
        print("  Promote staging → production? [y/N] (Timeout in 2 hours) ", end="", flush=True)
        t = threading.Thread(target=get_input, daemon=True)
        t.start()
        t.join(7200)  # 2 hours timeout
        
        if t.is_alive():
            print("\n[retrain] Approval timeout (2 hours) reached. Auto-declining.")
            approved = False
        else:
            approved = (answer[0] == "y")
